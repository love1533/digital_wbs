...
────────────────────────────────────────────────────────────
필수 환경변수:
  JIRA_URL          예) https://mycompany.atlassian.net
  JIRA_EMAIL        JIRA 계정 이메일
  JIRA_API_TOKEN    JIRA API 토큰 (atlassian.net → 계정 → 보안 → API 토큰)
...
사용법:
  ...
  python scripts/generate_wbs.py
"""
...
# 업무영역 자동 감지 키워드 (컴포넌트명 / 라벨 / 이슈 제목에서 탐색)
AREA_KEYWORDS = {
...
JIRA_FIELDS = [
    ...
    "customfield_10014",  # Epic Link (classic 프로젝트)
...
def priority_order(p: str) -> int:
    return {"Highest": 0, "High": 1, "Medium": 2, "Low": 3, "Lowest": 4}.get(p, 2)
...
class JiraClient:
    ...
    def search(self, jql: str, max_results: int = 1000) -> list:
        ...
        while True:
            ...
            if fetched == 0 or len(issues) >= data["total"] or len(issues) >= max_results:
                break
            start += fetched
        return issues
...
def build_phases(client: JiraClient) -> list:
    print(f"📡 JIRA 프로젝트 [{PROJECT_KEY}] 데이터 수집 중...")
    ...
    if not epics:
        epics = client.search(
            f'project = "{PROJECT_KEY}" AND issuetype = Epic ORDER BY created ASC'
        )
    print(f"  • 에픽: {len(epics)}개")
    ...
    # 3) 에픽별 이슈 그룹핑
    epic_map: dict[str, list] = {e["key"]: [] for e in epics}
    unassigned = []
    ...
    for issue in all_issues:
        ...
        if epic_key and epic_key in epic_map:
            epic_map[epic_key].append(issue)
        else:
    ...
    for i, epic in enumerate(epics):
        ef     = epic["fields"]
        issues = sorted(epic_map.get(epic["key"], []),
                        key=lambda x: priority_order((x["fields"].get("priority") or {}).get("name", "Medium")))
        tasks  = [make_task(iss, i + 1, j + 1) for j, iss in enumerate(issues)]
        done_c = sum(1 for t in tasks if t["status"] == "done")
        phases.append({
            "id":         f"P{i+1}",
            "name":       ef.get("summary", "Unknown Epic"),
            "jiraKey":    epic["key"],
            "jiraUrl":    f"{JIRA_URL}/browse/{epic['key']}",
            "status":     map_status(ef["status"]["name"]),
            "duedate":    ef.get("duedate") or "",
            "tasks":      tasks,
            "taskCount":  len(tasks),
            "doneCount":  done_c,
        })
...
.gate{{background:#1e293b;border:1px solid #334155;border-radius:1rem;padding:2.5rem 3rem;width:360px;text-align:center}}
...
.gate button{{width:100%;padding:.7rem;background:#1d4ed8;color:#fff;border:none;border-radius:.5rem;font-size:.9rem;cursor:pointer;transition:background .15s}}
.gate button:hover{{background:#2563eb}}
.gate .err{{font-size:.72rem;color:#f87171;margin-top:.5rem;min-height:1rem}}
.gate .lock{{font-size:2rem;margin-bottom:1rem}}

/* ─ Header ─ */
#main{{display:none}}
header{{background:#1e293b;border-bottom:1px solid #334155;padding:.85rem 1.5rem;display:flex;align-items:center;gap:.6rem;flex-wrap:wrap}}
header h1{{font-size:1.1rem;font-weight:700;color:#38bdf8;flex:1}}
.badge{{font-size:.65rem;padding:.15rem .55rem;border-radius:9999px;border:1px solid}}
...
.sync-info{{font-size:.65rem;color:#475569;margin-left:auto;display:flex;align-items:center;gap:.4rem}}
.dot{{width:6px;height:6px;border-radius:50%;background:#4ade80;animation:pulse 2s infinite}}
@keyframes pulse{{0%,100%{{opacity:1}}50%{{opacity:.4}}}}
.jira-link{{display:inline-flex;align-items:center;gap:.3rem;color:#94a3b8;font-size:.7rem;text-decoration:none;border:1px solid #334155;padding:.2rem .55rem;border-radius:.3rem;transition:all .15s}}
.jira-link:hover{{color:#38bdf8;border-color:#38bdf8}}

/* ─ Stats ─ */
.stats{{display:grid;grid-template-columns:repeat(5,1fr);gap:.6rem;padding:.85rem 1.5rem}}
@media(max-width:680px){{.stats{{grid-template-columns:repeat(3,1fr)}}}}
.sc{{background:#1e293b;border:1px solid #334155;border-radius:.5rem;padding:.6rem .8rem}}
.sc-l{{font-size:.6rem;color:#475569;text-transform:uppercase;letter-spacing:.04em;margin-bottom:.25rem}}
.sc-v{{font-size:1.15rem;font-weight:700;color:#38bdf8}}

/* ─ Toolbar ─ */
.toolbar{{background:#1a2332;border-bottom:1px solid #334155;padding:.5rem 1.5rem;display:flex;gap:.4rem;flex-wrap:wrap;align-items:center}}
.tb{{background:#0f172a;color:#94a3b8;border:1px solid #334155;border-radius:.25rem;padding:.22rem .55rem;font-size:.7rem;cursor:pointer;transition:all .15s}}
.tb:hover,.tb.on{{color:#38bdf8;border-color:#38bdf8;background:#1e3a5f}}
.prog{{margin-left:auto;display:flex;align-items:center;gap:.5rem;font-size:.7rem;color:#64748b}}
.pbar{{width:100px;height:4px;background:#1e293b;border-radius:2px;overflow:hidden}}
.pfill{{height:100%;background:linear-gradient(90deg,#22c55e,#38bdf8);border-radius:2px}}

/* ─ Phase blocks ─ */
.wbs{{padding:0 1.5rem 2rem}}
.ph{{background:#1e293b;border:1px solid #334155;border-radius:.6rem;margin-bottom:.7rem;overflow:hidden}}
.ph-hdr{{display:flex;align-items:center;gap:.6rem;padding:.7rem 1rem;cursor:pointer;user-select:none;transition:background .15s}}
.ph-hdr:hover{{background:#263347}}
.chv{{font-size:.65rem;color:#64748b;transition:transform .2s;width:12px}}
.chv.o{{transform:rotate(90deg)}}
.ph-id{{font-size:.65rem;font-weight:700;padding:.1rem .4rem;border-radius:.2rem;min-width:2.2rem;text-align:center}}
.ph-name{{font-weight:600;font-size:.85rem;flex:1}}
.ph-stat{{font-size:.65rem;color:#64748b;display:flex;align-items:center;gap:.4rem}}
.mbar{{width:48px;height:3px;background:#0f172a;border-radius:2px;overflow:hidden}}
.mfill{{height:100%;border-radius:2px}}

/* ─ Task rows ─ */
.rows{{border-top:1px solid #1a2332}}
.tr{{display:flex;align-items:flex-start;gap:.5rem;padding:.4rem .7rem;margin:.08rem .3rem;border-radius:.3rem;transition:background .1s}}
.tr:hover{{background:#1a2540}}
.status-dot{{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:.35rem}}
.s-done{{background:#22c55e}}.s-doing{{background:#38bdf8;animation:pulse 2s infinite}}
.s-review{{background:#fbbf24}}.s-todo{{background:#334155}}
.tbody{{flex:1;min-width:0}}
.tr1{{display:flex;align-items:center;gap:.4rem;flex-wrap:wrap}}
.tid{{color:#334155;font-size:.62rem;font-family:monospace;flex-shrink:0;min-width:3rem}}
.tkey{{font-size:.65rem;font-family:monospace;color:#475569;text-decoration:none;flex-shrink:0}}
.tkey:hover{{color:#38bdf8;text-decoration:underline}}
.tname{{font-size:.8rem;color:#cbd5e1;flex:1;min-width:0}}
.tname.done{{text-decoration:line-through;color:#475569}}
.area-tag{{font-size:.58rem;padding:.07rem .32rem;border-radius:.2rem;font-weight:600;flex-shrink:0;border:1px solid}}
.stag{{font-size:.58rem;padding:.07rem .32rem;border-radius:.2rem;font-weight:600;flex-shrink:0}}
.s-tag-done{{background:#052e16;color:#4ade80}}.s-tag-doing{{background:#1e3a5f;color:#38bdf8}}
.s-tag-review{{background:#2d1a00;color:#fbbf24}}.s-tag-todo{{background:#1a1a2e;color:#475569}}
.tassign{{font-size:.62rem;color:#334155;flex-shrink:0}}
.tdate{{font-size:.62rem;color:#334155;flex-shrink:0;margin-left:auto}}
.subs{{margin:.2rem 0 0 1rem;border-left:1px dashed #334155;padding-left:.6rem}}
.sub{{display:flex;align-items:center;gap:.35rem;padding:.15rem .15rem;font-size:.7rem;color:#475569}}
...
.sub a{{color:#475569;font-size:.62rem;font-family:monospace;text-decoration:none}}
...
<div id="overlay">
  <div class="gate">
    ...
    <input type="password" id="pw" placeholder="비밀번호" onkeydown="if(event.key==='Enter')doAuth()"/>
...
<header>
  ...
  <span class="badge b-green">진행중</span>
  <span class="badge b-amber">{pct}% 완료</span>
  <span class="sync-info">
    <span class="dot"></span>
    <span>최종 동기화: {last_update}</span>
  </span>
  <a class="jira-link" href="{JIRA_URL}/jira/software/projects/{PROJECT_KEY}/boards" target="_blank">
    <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M11.571 11.513H0a5.218 5.218 0 0 0 5.232 5.215h2.13v2.058A5.215 5.215 0 0 0 12.575 24V12.518a1.005 1.005 0 0 0-1.004-1.005zm5.723-5.756H5.757a5.215 5.215 0 0 0 5.214 5.214h2.129v2.058a5.218 5.218 0 0 0 5.215 5.214V6.762a1.005 1.005 0 0 0-1.021-1.005zM23.013 0H11.442a5.215 5.215 0 0 0 5.215 5.215h2.129v2.058A5.215 5.215 0 0 0 24 12.487V1.005A1.005 1.005 0 0 0 23.013 0z"/></svg>
    JIRA
  </a>
</header>
...
<div class="toolbar">
  ...
  <span style="width:1px;height:14px;background:#334155;margin:0 .15rem"></span>
  <button class="tb" onclick="expAll()">전체 펼치기</button>
  <button class="tb" onclick="colAll()">전체 접기</button>
  <div class="prog">
    <span id="ptxt">{pct}%</span>
    <div class="pbar"><div class="pfill" id="pfill" style="width:{pct}%"></div></div>
  </div>
</div>
...
const SC = {{done:'s-tag-done',doing:'s-tag-doing',review:'s-tag-review',todo:'s-tag-todo'}};
let curF = 'all';
const open = {{}};
...
function setF(f, btn) {{
  curF = f;
  ['all','todo','doing','review','done'].forEach(x=>document.getElementById('f-'+x)?.classList.toggle('on',x===f));
  render();
}}
...
function render() {{
  const total = PHASES.reduce((s,p)=>s+p.taskCount,0);
  const done  = PHASES.reduce((s,p)=>s+p.doneCount,0);
  ...
  PHASES.forEach(p => {{
    ...
    html += `<div class="ph">
      <div class="ph-hdr" onclick="tog('${{p.id}}')">
        ...
        ${{p.jiraKey ? `<a class="tkey" href="${{p.jiraUrl}}" target="_blank" onclick="event.stopPropagation()">${{p.jiraKey}}</a>` : ''}}
        ${{p.duedate ? `<span style="font-size:.62rem;color:#334155">🗓 ${{p.duedate}}</span>` : ''}}
        <div class="ph-stat">
    ...
    if (isOpen) {{
      ...
      tasks.forEach(t => {{
        ...
        const subsHtml = t.subs.length ? `<div class="subs">` +
          t.subs.map(s => `<div class="sub">
            <div class="sdot ${{s.status}}">&zwnj;</div>
            <a href="${{JIRA_BASE}}/browse/${{s.key}}" target="_blank">${{s.key}}</a>
            <span>${{s.name}}</span></div>`).join('') + `</div>` : '';

        html += `<div class="tr">
          <div class="status-dot s-${{t.status}}">&zwnj;</div>
          <div class="tbody">
            <div class="tr1">
  ...
  document.getElementById('wbs').innerHTML = html || '<p style="color:#475569;padding:2rem;text-align:center">조건에 맞는 이슈가 없습니다</p>';
}}
...
def main():
    ...
    print(f"✅ 수집 완료: 에픽 {len(phases)}개 / 이슈 {total}개 (완료 {done}개)")

    html = build_html(phases)

    os.makedirs(os.path.dirname(OUTPUT_PATH) or ".", exist_ok=True)
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"📄 HTML 생성: {OUTPUT_PATH}  ({len(html):,} bytes)")
